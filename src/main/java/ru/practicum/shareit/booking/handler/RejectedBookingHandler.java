...
public class RejectedBookingHandler extends BookingHandler {
    ...
    public List<Booking> handleRequest(List<Item> items, Long bookerId, BookingState bookingState, Sort sort) {
        if (bookingState == BookingState.REJECTED) {
            if (bookerId == null) {
                return bookingRepository.findAllByItemInAndStatus(items, BookingStatus.REJECTED, sort);
            } else {
                return bookingRepository.findBookingByIdAndStatus(bookerId, BookingStatus.REJECTED, sort);
            }
